...
const ScrollClipBgTextContext = React.createContext<
  ScrollClipBgTextContextValue | undefined
>(undefined);
export function useScrollClipBgTextContext() {
  const context = React.useContext(ScrollClipBgTextContext);
  if (!context) {
    throw new Error(
      "useScrollClipBgTextContext must be used within a ScrollClipBgTextContextProvider",
    );
  }
  return context;
}
export function ScrollClipBgText({
  spaceClass = "h-80",
  children,
  className,
  ...props
}: ScrollClipBgTextProps) {
...
export function ScrollClipBgTextWrap({
  yInput = [0, 1],
  yRange = [0, 320],
  style,
  ...props
